...
class Agent(nn.Module):
    ...
    def get_states(self, x, urnn_state, done):
        ...
        for h, d in zip(hidden, done):
            ...
            urnn_state = reset_mask * urnn_state + (1 - reset_mask) * self.urnn.initial_hidden(
                batch_size, device=urnn_state.device, dtype=urnn_state.dtype
            )
...
if __name__ == "__main__":
    ...
    run_name = f"cleanrl_ppo_atari_urnn_{args.env_id}_{args.seed}_{int(time.time())}"
    run_dir = f"./runs/cleanrl_ppo_atari_urnn_{args.env_id}/s{args.seed}"
    ...
    next_urnn_state = agent.urnn.initial_hidden(
        args.num_envs, device=device, dtype=torch.complex64
    )  # (batch_size, hidden_size) complex
